...
> 当前决策：先交付一个可直接运行的非官方原创中英双语交互教材。普通 `main` 推送即可构建和部署，不设置审批或法律门禁；无需先阅读上游原书即可完成本站学习。上游内容仅用于主题参考和链接对照；只有未来把上游原文、PDF、插图或代码直接纳入制品时，才单独处理对应的许可与署名问题。

## 1. 执行摘要

本项目将《Mathematical Foundations of Reinforcement Learning》相关主题设计为一个可独立学习、可实验、可复现、可离线使用的原创双语交互教材。
...
从技术和部署角度，本项目没有审批前置条件：它可以作为一个普通的纯静态双语学习网站开发、构建并部署。当前实现采用“非官方原创双语交互教材”范围——中英文讲解、Rust/Wasm 代码、图示和实验均由本项目独立编写，并链接上游作为主题参考；学习者无需先阅读上游原书。
...
第三章不复用第二章固定策略四状态 MRP 冒充最优控制，而是直接从第一章同一份 4×4 Grid World 派生每个状态和请求动作的联合转移结果。第一章会话模拟和第三章规划计算共享一套边界、危险格、目标、奖励及风扰动语义。

完整技术切片必须包含：

- 中英文成对的章节导览、策略改进、最优价值与策略、Bellman 最优方程、压缩映射、贪心最优策略、折扣/奖励/模型影响、综合检查点和实验页；
- 对有限状态动作、有界奖励和 $0\leq\gamma<1$ 的明确假设，以及 $v_*$ 唯一但最优策略可能不唯一的边界；
- 在 Rust 中先对随机转移结果求期望，再在请求动作之间取最大值，不能对风扰动后的实际动作逐结果取最大；
- 16 状态最优价值、5 个动作价值、全部贪心并列动作、同步 $T_*$ 扫描、当前残差、扫描上限及收敛/截断区分；
- 价值热图、动作备份账本、贪心策略箭头、最优值传播、折扣与风扰动对照、动态压缩不等式和模型审计；
- 第一轮和收敛值的独立 golden、转移概率归一化、终止值、并列动作、压缩界、参数校验、Worker/Wasm 重试、跨语言恢复及无 JavaScript 降级测试；
- 正确的第三章固定来源、canonical、双向 `hreflang`、本地搜索、GitHub Pages 子路径产物和公网 Worker/Wasm 冒烟。
...
3. [待办] 确定中英文数学编辑/译审责任人，并建立首版术语表；
4. [已完成] 在授权未决期间按双语原创伴读模式启动技术垂直切片；
5. [已完成] 初始化 Rust/Cargo 与 pnpm/VitePress monorepo；
6. [已完成] 建立 `mathrl-core`、`mathrl-wasm`、Worker 协议、双语内容 schema 和 locale manifest；
