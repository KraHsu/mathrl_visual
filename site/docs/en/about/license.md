...
material. The optional formal-release profile may add `LICENSE-CODE`,
`LICENSE-CONTENT`, and `NOTICE` files; these are not prerequisites for the
ordinary preview.

No page should be changed to a reproduction or translation of the book without
first checking the actual rights holder's permission. This boundary applies to
that future reproduction scope, not to the project's original explanations,
...
When permission is granted, the release record will name the rights holder, permitted formats, attribution text, license, effective date, and takedown contact. A request to correct or remove material should reference the page ID and content version so an old offline package can be identified.
